...
function handbookInfo(root) {
  ...
  // compared against or stamped with. Only candidates, no final yet, reads as untagged.
  const tag = releaseTag.latestReleaseTag(root);
  if (!tag) return { root, hasGit: true, untagged: true, version: 'v0' };
  return { root, hasGit: true, untagged: false, version: tag };
}
 ...
 * silently claims to be a release it is newer than. So `version` is `git describe --tags` (pre-release
 * tags excluded — release-tag.js, #334), which
...
function freezeVersion(root) {
  ...
  // Long form measured from the last FINAL tag (#334) — a tree sitting on `v1.3.0-rc.1` stamps
  // `v1.2.0-N-g<sha>`, never the candidate.
  const d = gitIn(root, releaseTag.describeArgs({ abbrev: null }));
