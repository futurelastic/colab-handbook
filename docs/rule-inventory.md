...
| 209 | A scheduler never promotes (`colab promote`) and never tags by itself, on any tier; an automatic tag is only ever a release skill's act in a coordinator session, where §6's release rung permits it — through `colab release cut` / `colab release finalize`. *(amended #335, #339)* | none | unknown | hard rule |
