...
- Branch names: `^([a-z0-9][a-z0-9-]*/[a-z0-9][a-z0-9-]*/)?(feat|fix|docs|chore|refactor|test|perf|design)/[a-z0-9._-]+$`,
  ending in the issue number(s): `feat/onboard-redesign-23`, or for a group
  `fix/import-fixes-115-114-113`. The optional `<login>/<machine>/` prefix
  (`ada/box-a/feat/onboard-redesign-23`) belongs only where `project.yml` declares
  `branchPrefix: machine`, and there `colab worktree new` adds it for you.
  Existing branches that violate this are grandfathered — **do not rename them**; some are live worktrees.
- **Branch off trunk, never off another feature branch.**
- Conventional Commits (`feat:` `fix:` `docs:` `chore:` `refactor:` `test:`
  `perf:` `design:`). Release notes are generated by grouping on these prefixes
  — an unprefixed commit is invisible in the changelog. The squash-subject
  picker's full ranked set (wider than this list, and what actually decides a
  multi-commit branch's headline) is in `CONVENTIONS.md` §4.
